...
namespace CRC
{
    public class Crc : HashAlgorithm
    {
        ...
        public static UInt64 ComputeHash(CrcAlgorithms alg, byte[] data)
        {
            var h = new Crc(CrcStdParams.StandartParameters[alg]).ComputeHash(data);
            UInt64 result = 0;
            for(int i=0;i<8;i++)
            {
                result |= (UInt32)(h[7 - i] << 8*i);
            }
            return result;
        }
        private static UInt32 swapOctetsUInt32(UInt32 toSwap)
        {
            UInt32 tmp = 0;
            tmp = toSwap >> 24;
        ...
        public static UInt64 ComputeHash(CrcAlgorithms alg, uint[] data)
        {
            var ms = new MemoryStream();
            var bw = new BinaryWriter(ms);
            foreach(uint d in data)
            {
                bw.Write(swapOctetsUInt32(d));
            }
            return ComputeHash(alg, ms.ToArray());
        }
    }
}